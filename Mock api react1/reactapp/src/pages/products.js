...
function ProductList(props) {
    ...
    function deleteProduct(id) {
        ...
        .then((response) => {
            ...
            // No need to parse response as it will be empty for DELETE requests
            return response.text();
        })
        .then(() => fetchProducts()) // Refresh the product list after successful deletion
...
function ProductForm(props) {
    ...
    useEffect(() => {
        // Fetch the products to determine the next available ID
        fetch("http://localhost:3004/products")
        .then((response) => {
            if(!response.ok){
                throw new Error("Unexpected Server Response");
        ...
        .then((data) => {
            // Find the maximum ID and increment it by 1 to get the next available ID
    ...
    function hanbleSubmit(event){
        ...
        // Add the next available ID to the product
        product.id = nextId;

        //form validations
        if(!product.name || !product.brand || !product.catageory || !product.price) {
    ...
    return(
        ...
        <div className="row">
            ...
            {errorMessage}
            <form onSubmit={(event) => hanbleSubmit(event)}>
                ...
                <div className="row mb-3">
                    ...
                    <div className="col-sm-8">
                        <select className="form-select" 
                            name="catageory"
                            defaultValue={props.product.catageory}> 
